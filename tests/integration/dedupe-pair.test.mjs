...
test("dedupe: two identical messages 100ms apart produce two lines", async () => {
  ...
  // Send two identical messages from different user IDs but the same
  // display name ~100ms apart.  Different IDs bypass the per-user
  // textwatcher cooldown; the same displayName + text is what the old
  // dedupe heuristic collapsed.  The new FIFO matching should produce
  // two separate lines.
  ...
  // Wait for both server echoes to arrive and dedupe to settle.
  ...
  assert.equal(occurrences, 2, "text should appear exactly twice, got " + occurrences + " in: " + desc);
});