...
test("dedupe: textwatcher pre-push + server echo = one line", async () => {
  ...
  // Scan all edits (including embed fields) for max occurrence count.
  // Other test runs inject WS frames that fill the feed; pagination
  // may push our message off the description into a field.
  ...
  assert.equal(maxOccurrences, 1, "text should appear exactly once in at least one edit, got max " + maxOccurrences + " in: " + bestDesc);
});